...
class Shop {
  ...
  updateQuality() {
    for (var i = 0; i < this.items.length; i++) {
      ...
      let done = this.dispatchUpdate(item);
    }
  ...
  dispatchUpdate(item) {
    ...
    if (
      staticItems.find(function(staticItem) {
        return item.name === staticItem;
      })
    ) {
      return;
    }
    if (
      increaseItems.find(function(increaseItem) {
        return item.name === increaseItem;
      })
    ) {
      return this.updateIncreaseItem(item);
    }
    if (
      passes.find(function(pass) {
